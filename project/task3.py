...
def rsm_to_fa(rsm: RecursiveAutomaton) -> FiniteAutomaton:
    ...
    states_to_int = {state: lbl for lbl, state in enumerate(states)}
        ...
        for frm, transition in enfa.dfa.to_dict().items():
            for symbol, to in transition.items():
                ...
                if var not in matrix:
                    matrix[var] = dok_matrix((len(states), len(states)), dtype=bool)
...
def transitive_closure(fa: FiniteAutomaton):
    ...
    front = sum(fa.matrix.values(), next(iter(fa.matrix.values())))
    prev = 0
